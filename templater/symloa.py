...
def __loa_sympaths(fpath:str)->list[list[str,str]]:
    try:
        return fu.__ldirs(fpath)
    except Exception as e:
        log.error_log(e.__str__())
        return [["", ""]]

# Load aarch symbols from path.
def __loa_syms(aarch_name:str, sympath_dirs:list[list[str, str]])->list[str, list[str,str]]:
    try:
        _aarch:str = [x[1] for x in sympath_dirs if x[0] == aarch_name][0]
        return [aarch_name, fu.__lfiles(_aarch)]
    except Exception as e:
        log.error_log(e.__str__())
        return ["", ["", ""]]

# Expand __AARCH tables.
def __exp_aarch(aarch_symbols:list[str, list[str, str]])->list[str, bu.__ARCH, list[str,str]]:
    try:
        aarch_f = [f[1] for f in aarch_symbols[1] if f[0] == f"{aarch_symbols[0]}.aarch"][0]
        dat:str = ""
        with fu.__open_r(aarch_f) as _f:
            dat = _f.read()
...
# Expand HOST aarch.
def __exp_haarch(aarch_name:str, fpath:str):
    NotImplemented